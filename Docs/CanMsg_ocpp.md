/*
  CanMsg.cpp - Gestion des messages CAN entrants et sortants
*/
  ...
  for (;;)
  {
    ...
    if (ACAN_ESP32::can.receive(frameIn))
    {
      ...
      else
      {
        ...
        case 0xE7: // Réception d’un aspect affiché par un satellite voisin
        {
          uint8_t sens = frameIn.data[0];  // 0 = horaire, 1 = anti-horaire
          ...
          {
            voisin->aspectRecu[sens] = aspect;

#ifdef DEBUG
            debug.printf("[CanMsg] Aspect reçu de Sat %d → sens %s → aspect 0x%X\n",
                         idSatExpediteur,
                         (sens == 0 ? "horaire" : "anti-horaire"),
                         aspect);
#endif
          }
          else
          {
#ifdef DEBUG
            debug.printf("[CanMsg] Aspect ignoré : expéditeur %d non reconnu comme voisin SP1/SM1\n", idSatExpediteur);
#endif
          }
        }
        break;
        ...
        case 0xE9:
          {
              /*****************************************************************************************************
               * 0xE9 — Commande d’aiguillage reçue via CAN
               *
               * frameIn.data[0] = ID du SA destinataire
               * frameIn.data[1] = index de l’aiguille (0..N)
               *
               * ⚠️ IMPORTANT :
               * Le SA ne pilote plus les servos localement.
               * Il doit maintenant transmettre la commande au bon EXSA via RS485.
               *
               * Le bus RS485 est commun aux deux EXSA (H et AH).
               * Chaque EXSA est universel et doit ignorer les trames qui ne lui sont pas destinées.
               *
               * → On détermine donc ici à quel EXSA appartient l’aiguille :
               *      - EXSA côté H  → adresse = 0
               *      - EXSA côté AH → adresse = 1
               *
               * → Puis on envoie une trame F0 (servoMove) adressée au bon EXSA.
               *****************************************************************************************************/
              ...
              // Envoyer la commande de mouvement réel (F0) au bon EXSA
