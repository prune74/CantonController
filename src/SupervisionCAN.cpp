...
namespace SupervisionCAN
{
    ...
    void envoyerEtatCAN(Canton *canton)
    {
        ...
        CC_LOG_TRACE("[CAN][CC] Envoi UPDATE_VOISINS : busy=%d SP1=%d SM1=%d accesSP1=%d busySP1=%d accesSM1=%d busySM1=%d\n",
                     canton->busy(),
                     sp1->ID(), sm1->ID(),
                     sp1->acces(), sp1->busy(),
                     sm1->acces(), sm1->busy());
    ...
    /* ============================================================================
     * envoyerRechercheWagon()
     * ==========================================================================*/
    void envoyerRechercheWagon(Canton *canton, uint16_t destId, uint16_t locoId, uint8_t index)
    {
        uint8_t locoHigh = (locoId >> 8) & 0xFF;
        uint8_t locoLow = locoId & 0xFF;

        CC_LOG_INFO("[CAN][CC] Envoi RECHERCHE_WAGON : dest=%u rame=%u index=%u\n",
                    destId, locoId, index);

        CC_CAN::sendMsg(
            0,
            (uint16_t)Cmd_CC_to_CC::RECHERCHE_WAGON,
            0,
            canton->ID(),
            destId,
            locoHigh,
